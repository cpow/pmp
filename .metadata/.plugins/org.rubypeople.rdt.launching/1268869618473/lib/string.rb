...
-----------------
     ...
     empty?, end_regexp, ends_with?, eql?, expand_ch_hash, ext, getbyte,
     gsub, gsub!, hash, hex, include?, index, initialize_copy, insert,
     inspect, intern, is_binary_data?, is_complex_yaml?, iseuc, issjis,
     isutf8, jcount, jlength, jsize, kconv, length, ljust, lstrip,
     lstrip!, match, mbchar?, next, next!, oct, original_succ,
     original_succ!, pathmap, pathmap_explode, pathmap_partial,
     pathmap_replace, quote, replace, reverse, reverse!, rindex, rjust,
     rstrip, rstrip!, scan, scanf, size, slice, slice!, split, squeeze,
     squeeze!, starts_with?, strip, strip!, sub, sub!, succ, succ!, sum,
     swapcase, swapcase!, to_blob, to_date_lex, to_date_time_lex, to_f,
     to_i, to_i_lex, to_libxml_doc, to_s, to_str, to_sym, to_time_lex,
     to_yaml, toeuc, tojis, tosjis, toutf16, toutf8, tr, tr!, tr_s,
     tr_s!, unpack, upcase, upcase!, upto
